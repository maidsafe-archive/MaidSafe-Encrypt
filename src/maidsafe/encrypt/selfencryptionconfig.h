...
const boost::uint16_t kMinChunks(3);
const boost::uint16_t kMaxChunks(40);
const boost::uint64_t kDefaultChunkSize(262144);
// static_cast<boost::uint16_t>(kDefaultChunkletSize) MUST be a multiple of
// 2*IV for AES encryption, i.e. multiple of 32.
const boost::uint16_t kDefaultChunkletSize(16384);
const boost::uint16_t kMinChunkletSize(32);
...
#endif  // MAIDSAFE_ENCRYPT_SELFENCRYPTIONCONFIG_H_
