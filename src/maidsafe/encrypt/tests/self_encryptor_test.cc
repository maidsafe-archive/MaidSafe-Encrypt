...
TEST_P(BasicOffsetTest, BEH_EncryptDecrypt) {
  ...
  self_encryptor_.reset(new SureFileSelfEncryptor(data_map_, sure_file_store_, num_procs_));
...
INSTANTIATE_TEST_CASE_P(
    ...
    testing::Values(2, kMinChunkSize - 1, kMinChunkSize, kMinChunkSize + 1, 3 * kMinChunkSize - 1,
                    ...
                    3 * kDefaultChunkSize, 3 * kDefaultChunkSize + 1, 4 * kDefaultChunkSize - 1,
                    4 * kDefaultChunkSize, 4 * kDefaultChunkSize + kMinChunkSize - 1,
...
INSTANTIATE_TEST_CASE_P(
    ...
    testing::Values(1, kMinChunkSize - 1, kMinChunkSize, kMinChunkSize + 1, 3 * kMinChunkSize - 1,
                    3 * kMinChunkSize, 3 * kMinChunkSize + 1, kDefaultChunkSize - 1,
                    kDefaultChunkSize, kDefaultChunkSize + 1));
...
TEST_F(BasicTest, BEH_NewRead) {
  ...
  {
    SureFileSelfEncryptor self_encryptor(data_map2, sure_file_store_, num_procs_);
...
TEST_F(BasicTest, BEH_WriteRandomSizeRandomPosition) {
  ...
  SureFileSelfEncryptor self_encryptor(data_map_, sure_file_store_, num_procs_);
...
TEST_F(BasicTest, BEH_DeleteStoredChunkFromDisk) {
  ...
  fs::directory_iterator it(*test_dir_);
...
TEST_F(BasicTest, BEH_TruncateIncreaseScenario1) {
  ...
  SureFileSelfEncryptor temp_self_encryptor(data_map_, sure_file_store_, num_procs_);
...
TEST_F(BasicTest, BEH_DifferentDataMapSameChunk) {
  ...
  {
    SureFileSelfEncryptor self_encryptor_1(data_map_1, sure_file_store_, num_procs_);
    SureFileSelfEncryptor self_encryptor_2(data_map_2, sure_file_store_, num_procs_);
