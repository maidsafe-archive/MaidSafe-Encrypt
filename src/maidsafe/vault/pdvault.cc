...
PDVault::PDVault(const std::string &pmid_public,
    ...
    : K_(k),
      ...
      vault_chunkstore_(new VaultChunkStore((vault_dir / "Chunkstore").string(),
                        available_space, used_space)),
      kad_ops_(new KadOps(&transport_handler_, &channel_manager_, kad::VAULT,
                          pmid_private, pmid_public, port_forwarded, use_upnp,
                          K_, vault_chunkstore_)),
      vault_service_(),
...
void PDVault::Start(bool first_node) {
  ...
  pmid_ = SHA512String(pmid_public_ + signed_pmid_public_);
  validator_.set_id(pmid_);
  kad_ops_->set_signature_validator(&validator_);
  ...
  int chunkstore_result =
      vault_chunkstore_->HashCheckAllChunks(true, &failed_keys);

  // Block until Kademlia Join completed
  {
    boost::mutex::scoped_lock lock(mutex);
    while (kad_ops_result == kPendingResult)
      cond_var.wait(lock);
  }
  ...
  port_ = kad_ops_->Port();
  routing_table_ = (*base::PublicRoutingTable::GetInstance())
        [boost::lexical_cast<std::string>(port_)];
  RegisterMaidService();
  if (vault_service_logic_.Init(pmid_, pmid_public_, signed_pmid_public_,
                                pmid_private_)) {
    SetVaultStatus(kVaultStarted);
    ...
    // will also sync with other vaults in the network afterwards
    sync_done_ = false;
    maidsafe_join_thread_ =
        boost::thread(&PDVault::JoinMaidsafeNet, this);
  }
#ifdef DEBUG
  printf("\tIn PDVault::Start, after Join.  On port %u\n", port_);
#endif
}
...
VaultStatus PDVault::vault_status() {
  boost::mutex::scoped_try_lock lock(vault_status_mutex_);
...
bool PDVault::WaitForStartup(const boost::uint16_t &timeout) {
  const boost::system_time cutoff = boost::get_system_time() +
                                    boost::posix_time::seconds(timeout);
  boost::mutex::scoped_lock lock(vault_status_mutex_);
  while (vault_status_ != kVaultStarted &&
         boost::detail::get_milliseconds_until(cutoff) > 0) {
    vault_status_cond_.timed_wait(lock, cutoff);
  }
#ifdef DEBUG
  if (boost::detail::get_milliseconds_until(cutoff) == 0)
    printf("PDVault::WaitForStartup - timed out\n");
#endif
  return vault_status_ == kVaultStarted;
}

/*
void PDVault::SyncVault(kad::VoidFunctorOneString cb) {
  ...
  if (!data->chunk_names.empty()) {
    ...
    UpdateChunkResponse local_result;
  ...
  for (int i = 0; i < result_msg.values_size(); ++i) {
    ...
    if (signed_value.ParseFromString(result_msg.values(i))) {
      ...
      if (remote.ParseFromString(contact_info) &&
          remote.node_id() != kad_ops_->node_id()) {
        correct_info = true;
        kad_ops_->Ping(remote,
...
void PDVault::ValidityCheck(const std::string &chunk_name,
  ...
  if (random_data == "" ||
    ...
    random_data_ =
        base::RandomString((base::RandomUint32() % 512) + 512);
  else
  ...
  vault_rpcs_->ValidityCheck(validity_check_args->chunk_name_,
      ...
      udt_transport_.transport_id(), validity_check_response.get(), controller,
      callback);
}
...
void PDVault::ValidityCheckCallback(
  ...
  std::string local_hash_content_(SHA512String(local_content_ +
      validity_check_args->random_data_));
...
void PDVault::IterativeSyncVault_SyncChunk(
  ...
  ValidityCheckResponse result_msg;
  if (!result_msg.ParseFromString(result) ||
    ...
    boost::shared_ptr<GetChunkResponse>
        get_chunk_response(new GetChunkResponse());
    google::protobuf::Closure* callback = google::protobuf::NewCallback(this,
    ...
    vault_rpcs_->GetChunk(synch_args->chunk_name_, ip, port,
        ...
        synch_args->chunk_holder_.rendezvous_port(), udt_transport_.transport_id(),
...
void PDVault::RepublishChunkRef(kad::VoidFunctorOneString cb) {
...
void PDVault::IterativePublishChunkRef(
  ...
  if (data->chunk_names.empty()) {
    ...
    std::string signed_request = RSASign(SHA512String(
        pmid_public_ + signed_pmid_public_ + non_hex_chunk_name),
        pmid_private_);
    kad::SignedValue signed_value;
    signed_value.set_value(pmid_);
    signed_value.set_value_signature(RSASign(pmid_, pmid_private_));
    ...
    kad_ops_->StoreValue(chunk_name,
                       signed_value,
                       sr,
                       86400,
...
void PDVault::FindChunkRef(boost::shared_ptr<struct LoadChunkData> data) {
  ...
  kad_ops_->FindValue(data->chunk_name, false,
...
void PDVault::CheckChunk(boost::shared_ptr<GetArgs> get_args) {
  ...
  bool local = kad_ops_->AddressIsLocal(get_args->chunk_holder_);
  std::string ip = get_args->chunk_holder_.host_ip();
  boost::uint16_t port = get_args->chunk_holder_.host_port();
  if (local) {
  ...
  if (!check_chunk_response->IsInitialized() ||
    ...
    if (!get_args->data_->is_active) {
      ...
      if (get_args->data_->get_msgs) {
          boost::shared_ptr<GetBPMessagesResponse>
              get_messages_response(new GetBPMessagesResponse());
  ...
  if (data->retry < kMaxChunkLoadRetries) {
    ...
    for (int i = 0; i < static_cast<int>(data->chunk_holders.size()); ++i) {
      ...
      for (int j = 0; j < static_cast<int>(data->failed_chunk_holders.size())
            && send; ++j)
        if (remote.Equals(data->failed_chunk_holders[j]))
          send = false;
      if (send) {
        boost::shared_ptr<GetArgs> get_args_(new GetArgs(remote, data));
        CheckChunk(get_args_);
...
void PDVault::SwapChunk(const std::string &chunk_name,
  ...
  boost::shared_ptr<SwapChunkArgs> swap_chunk_args(
      ...
      rendezvous_port, cb));
  boost::shared_ptr<SwapChunkResponse>
      swap_chunk_response(new SwapChunkResponse());
  google::protobuf::Closure* callback = google::protobuf::NewCallback(this,
  ...
  if (!vault_chunkstore_->Load(swap_chunk_args->chunkname_,
      &chunkcontent1) != 0) {
    SwapChunkResponse local_result;
    std::string local_result_str("");
...
void PDVault::SwapChunkAcceptChunk(
  ...
  std::string signed_request = RSASign(SHA512String(
      pmid_public_ + signed_pmid_public_ + chunk_name), pmid_private_);
  kad::SignedValue signed_value;
  signed_value.set_value(pmid_);
  signed_value.set_value_signature(RSASign(pmid_, pmid_private_));
  kad::SignedRequest sr;
  ...
  sr.set_signed_public_key(signed_pmid_public_);
  sr.set_signed_request(signed_request);
  kad_ops_->StoreValue(swap_chunk_args->chunkname_, signed_value, sr,
      &pdv_dummy_callback);
  SwapChunkResponse local_result;
  std::string local_result_str("");
  local_result.set_request_type(1);
...
int PDVault::AmendAccount(const boost::uint64_t &space_offered) {
  ...
  std::string account_name(SHA512String(pmid_ + kAccount));
  ...
  if (ContactWithinClosest(account_name, our_details_, data->contacts)) {
  ...
  mutable_signed_size->set_signature(RSASign(
      boost::lexical_cast<std::string>(space_offered), pmid_private_));
...
void PDVault::JoinMaidsafeNet() {
  boost::mutex::scoped_lock lock(vault_status_mutex_);
  sync_done_ = sync_succeeded_ = false;
...
#ifdef DEBUG
  printf("In PDVault::JoinMaidsafeNet (%s), set space offered to %llu "
         "on attempt #%d.\n", HexSubstr(pmid_).c_str(),
  ...
  bool callbacked1(false), callbacked2(false);
  rpcprotocol::Controller controller1, controller2;
  GetSyncDataRequest get_sync_data_req1;
  get_sync_data_req1.set_pmid(pmid_);
  get_sync_data_req1.set_public_key(pmid_public_);
  get_sync_data_req1.set_public_key_signature(signed_pmid_public_);
  GetSyncDataRequest get_sync_data_req2(get_sync_data_req1);
  GetSyncDataResponse get_sync_data_resp1, get_sync_data_resp2;

  {
    base::PublicRoutingTableTuple closest_tuple = close_peers.front();
    std::string signature = RSASign(SHA512String(
        signed_pmid_public_ + closest_tuple.kademlia_id), pmid_private_);
    get_sync_data_req1.set_request_signature(signature);

    google::protobuf::Closure* done = google::protobuf::NewCallback(this,
          &PDVault::GetSyncDataCallback, &callbacked1,
          std::pair<boost::mutex*, boost::condition_variable*>
              (&vault_status_mutex_, &vault_status_cond_));

    kad::Contact peer(closest_tuple.kademlia_id, closest_tuple.host_ip,
        closest_tuple.host_port, closest_tuple.host_ip,
  ...
  if (single_peer) {
    ...
    base::PublicRoutingTableTuple furthest_tuple = close_peers.back();
    std::string signature = RSASign(SHA512String(
        signed_pmid_public_ + furthest_tuple.kademlia_id), pmid_private_);
    get_sync_data_req2.set_request_signature(signature);

    google::protobuf::Closure* done = google::protobuf::NewCallback(this,
          &PDVault::GetSyncDataCallback, &callbacked2,
          std::pair<boost::mutex*, boost::condition_variable*>
          (&vault_status_mutex_, &vault_status_cond_));

    kad::Contact peer(furthest_tuple.kademlia_id, furthest_tuple.host_ip,
        furthest_tuple.host_port, furthest_tuple.host_ip,
  ...
  // wait for the RPCs to return, or the vault to be stopped
  while (vault_status_ == kVaultStarted && !(callbacked1 && callbacked2))
    vault_status_cond_.wait(lock);

  if (vault_status_ != kVaultStarted) {
    // kill remaining RPCs
    channel_manager_.CancelPendingRequest(controller1.request_id());
    if (!single_peer)
      channel_manager_.CancelPendingRequest(controller2.request_id());
#ifdef DEBUG
    printf("In PDVault::JoinMaidsafeNet (%s), failed to synchronise, vault is "
           "offline.\n", HexSubstr(pmid_).c_str());
#endif
    sync_done_ = true;
    vault_status_cond_.notify_all();
  ...
  vault_service_->AddStartupSyncData(get_sync_data_resp1);
  if (!single_peer)
    vault_service_->AddStartupSyncData(get_sync_data_resp2);
...
void PDVault::GetSyncDataCallback(
  ...
  (*done) = true;
  sync.second->notify_all();
}

bool PDVault::WaitForSync() {
  boost::mutex::scoped_lock lock(vault_status_mutex_);
  while (vault_status_ == kVaultStarted && !sync_done_)
    vault_status_cond_.wait(lock);
  return sync_done_ && sync_succeeded_;
}
