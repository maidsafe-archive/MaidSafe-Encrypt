...
class FunctionalClientControllerTest : public testing::Test {
 protected:
  FunctionalClientControllerTest()
      : cc_dir_(file_system::FileSystem::TempDir() + "/maidsafe_TestCC_"
                + base::RandomString(6)),
        vault_daemon_(0, cc_dir_),
        cc_(),
        authentication_(),
        ss_(),
        dir1_(),
  ...
  void SetUp() {
    ...
    int timeout(60000);
    int count(0);
    fs::path kadconfig_dest(cc_dir_ + "/.kadconfig");
    while (!fs::exists(kadconfig_dest) && count < timeout) {
      try {
        fs::copy_file(".kadconfig", kadconfig_dest);
      }
      catch(const std::exception &e) {
        printf("Error: %s\n", e.what());
      }
    ...
    vcp_.space = 100;
    vcp_.directory = cc_dir_ + "/VaultChunkstore";
  }
  ...
  void TearDown() {
    try {
      ...
      if (!final_dir_.empty() && fs::exists(final_dir_))
