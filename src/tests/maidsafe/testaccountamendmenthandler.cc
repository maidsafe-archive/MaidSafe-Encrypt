...
TEST_F(AccountAmendmentHandlerTest, BEH_MAID_AAH_AssessAmendment) {
  ...
  // Set account name as far as possible from our ID so we don't get added to
  // vector of close nodes in vsl_.HandleFindKNodesResponse
  std::string far_account_name =
  ...
  result = aah_.AssessAmendment(far_account_name, 2, 1000, true, pending2,
      &test_amendment);
  ASSERT_EQ(kAccountAmendmentUpdated, result);
  ASSERT_EQ(far_account_name, test_amendment.account_name);
  ASSERT_EQ(good_pmids_.size(),
  ...
  result = aah_.AssessAmendment(far_account_name, 2, 1000, true, pending4,
      &test_amendment);
...
TEST_F(AccountAmendmentHandlerTest, BEH_MAID_AAH_CreateNewAmendment) {
  ...
  for (int i = 0; i < kTestRuns; ++i) {
    ...
    AccountAmendment amendment(far_account_name, 2, 1000, true, pending);
    amendment.account_name = far_account_name;
...
//  printf("Run %i\n", test_run);
  ...
  // Call 6 - Add new amendment but send requests with bad PMIDs
  ++test_run;
//  printf("Run %i\n", test_run);
  aah_.amendments_.clear();
  request = kDefaultRequest;
  ...
  std::vector<maidsafe::AmendAccountRequest> requests;
  std::vector<maidsafe::AmendAccountResponse> responses;
  std::vector<google::protobuf::Closure*> callbacks1;
  for (int i = 0; i < kad::K; ++i) {
    ...
    sz->set_pmid(crypto_.Hash(good_pmids_.at(i), "", crypto::STRING_STRING,
        false));
    requests.push_back(req);
    responses.push_back(resp);
    google::protobuf::Closure *done1 = google::protobuf::NewCallback(&cbh,
        &test_aah::CallbacksHolder::callback);
    callbacks1.push_back(done1);
  }
  ASSERT_EQ(kSuccess, aah_.ProcessRequest(&requests.at(0), &responses.at(0),
            callbacks1.at(0)));
  ASSERT_EQ(size_t(1), aah_.amendments_.size());
  for (size_t i = 1; i < kMaxRepeatedAccountAmendments; ++i) {
    ASSERT_EQ(kSuccess, aah_.ProcessRequest(&requests.at(i), &responses.at(i),
              callbacks1.at(i)));
  }
  for (int i = kMaxRepeatedAccountAmendments; i < kad::K; ++i) {
    ASSERT_EQ(kAmendAccountCountError, aah_.ProcessRequest(&requests.at(i),
              &responses.at(i), callbacks1.at(i)));
  }
  ASSERT_EQ(kMaxRepeatedAccountAmendments, aah_.amendments_.size());
  ASSERT_TRUE(test_aah::CheckAcc(account_owner, offer, v_used, acc_used, &ah_));
  for (size_t i = 0; i < kad::K; ++i) {
    ASSERT_TRUE(responses.at(i).IsInitialized());
    ASSERT_EQ(kNack, static_cast<int>(responses.at(i).result()));
  }
  ASSERT_EQ(size_t(11), ah_.accounts_.size());

  // Call 7 - Add new amendment and populate FindNodes result with good nodes.
...
//  printf("Run %i\n", test_run);
  ...
  requests.clear();
  responses.clear();
  std::vector<google::protobuf::Closure*> callbacks2;
  for (int i = 0; i < kad::K; ++i) {
    maidsafe::AmendAccountRequest req(kDefaultRequest);
    maidsafe::AmendAccountResponse resp(kDefaultResponse);
    maidsafe::SignedSize *sz = req.mutable_signed_size();
  ...
  while (cbh.called_back_count() != 39)
  ...
  ASSERT_TRUE(aah_.amendments_.size() == size_t(2) || aah_.amendments_.empty());
