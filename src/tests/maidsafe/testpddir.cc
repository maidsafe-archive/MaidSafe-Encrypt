...
TEST_F(PdDirTest, BEH_MAID_ConnectDb) {
  ...
  {
    int result_ = -1;
    boost::scoped_ptr<PdDir> da_(new PdDir("Non-existant db", CONNECT, &result_));
...
TEST_F(PdDirTest, BEH_MAID_AddElement) {
  ...
  PrepareMetaDataMap(-2, file_name_, REGULAR_FILE, file_hash_, "Stats1", "Tag1", 0, 1, 2, 0, 0, ser_mdm1_);
  PrepareDataMap(file_hash_, ser_dm_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm1_, ser_dm_)) << "File was not added to db1.";

  // add directory
  std::string dir_name_ = "Directory";
  std::string dir_key_="Dir Key";
  std::string ser_mdm2_="";
  PrepareMetaDataMap(-2, dir_name_, DIRECTORY, "", "Stats2", "Tag2", 0, 0, 3, 0, 0, ser_mdm2_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm2_, "", dir_key_)) << "Directory was not added to db1.";
}
...
TEST_F(PdDirTest, BEH_MAID_GetIdFromName) {
  ...
  PrepareMetaDataMap(-2, file_name1_, REGULAR_FILE, file_hash1_, "Stats1", "Tag1", 0, 1, 2, 0, 0, ser_mdm1_);
  PrepareDataMap(file_hash1_, ser_dm1_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm1_, ser_dm1_)) << "File1 was not added to db1.";

  // add file2
  std::string file_name2_ = "File2.file";
  std::string file_hash2_ = "File Hash2";
  std::string ser_mdm2_="", ser_dm2_="";
  PrepareMetaDataMap(-2, file_name2_, REGULAR_FILE, file_hash2_, "Stats2", "Tag2", 0, 3, 4, 0, 0, ser_mdm2_);
  PrepareDataMap(file_hash2_, ser_dm2_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm2_, ser_dm2_)) << "File2 was not added to db1.";

  // add file3
  std::string file_name3_ = "File3.file";
  std::string file_hash3_ = "File Hash3";
  std::string ser_mdm3_="", ser_dm3_="";
  PrepareMetaDataMap(-2, file_name3_, REGULAR_FILE, file_hash3_, "Stats3", "Tag3", 0, 5, 6, 0, 0, ser_mdm3_);
  PrepareDataMap(file_hash3_, ser_dm3_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm3_, ser_dm3_)) << "File3 was not added to db1.";

  // get ids
  ASSERT_EQ(2, da_->GetIdFromName(file_name2_)) << "Returned wrong id for file2.";
  ASSERT_EQ(3, da_->GetIdFromName(file_name3_)) << "Returned wrong id for file3.";
  ASSERT_EQ(1, da_->GetIdFromName(file_name1_)) << "Returned wrong id for file1.";
  ASSERT_TRUE(da_->GetIdFromName("Non-existant File")<0) << "Returned ID for file which doesn't exist.";
}
...
TEST_F(PdDirTest, BEH_MAID_GetDirKey) {
  ...
  PrepareMetaDataMap(-2, dir_name1_, DIRECTORY, "", "Stats1", "Tag1", 0, 0, 1, 0, 0, ser_mdm1_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm1_, "", dir_key1_)) << "Directory1 was not added to db1.";

  // add directory2
  std::string dir_name2_ = "Directory2";
  std::string dir_key2_="Dir Key2";
  std::string ser_mdm2_="";
  PrepareMetaDataMap(-2, dir_name2_, DIRECTORY, "", "Stats2", "Tag2", 0, 0, 2, 0, 0, ser_mdm2_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm2_, "", dir_key2_)) << "Directory2 was not added to db1.";

  // add directory3
  std::string dir_name3_ = "Directory3";
  std::string dir_key3_="Dir Key3";
  std::string ser_mdm3_="";
  PrepareMetaDataMap(-2, dir_name3_, DIRECTORY, "", "Stats3", "Tag3", 0, 0, 3, 0, 0, ser_mdm3_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm3_, "", dir_key3_)) << "Directory3 was not added to db1.";

  // get dir keys
  std::string dir_key_recovered1_="", dir_key_recovered2_="", dir_key_recovered3_="", dir_key_non_existant_="";
  ASSERT_EQ(0, da_->GetDirKey(dir_name3_, &dir_key_recovered3_)) << "Failed to recover dir key for dir3.";
  ASSERT_EQ(dir_key3_, dir_key_recovered3_) << "Returned wrong dir key for dir3.";
  ASSERT_EQ(0, da_->GetDirKey(dir_name1_, &dir_key_recovered1_)) << "Failed to recover dir key for dir1.";
  ASSERT_EQ(dir_key1_, dir_key_recovered1_) << "Returned wrong dir key for dir1.";
  ASSERT_EQ(0, da_->GetDirKey(dir_name2_, &dir_key_recovered2_)) << "Failed to recover dir key for dir2.";
  ASSERT_EQ(dir_key2_, dir_key_recovered2_) << "Returned wrong dir key for dir2.";
...
TEST_F(PdDirTest, BEH_MAID_DmExistsFromHash) {
  ...
  ASSERT_FALSE(da_->DataMapExists("Non-existant File Hash")) << "Found non-existant Dm.";
}
...
TEST_F(PdDirTest, BEH_MAID_RemoveElement) {
  ...
  // get the elements' ids.  I mean, they're definitely there, but I'd better just double check.
  ...
  ASSERT_TRUE(da_->GetIdFromName(file_name_)<0) << "Didn't remove file.";
  ...
  ASSERT_TRUE(da_->GetIdFromName(dir_name_)<0) << "Didn't remove dir.";
  ASSERT_NE(0, da_->RemoveElement("Non-existant File")) << "Removed non-existant file.";
}
...
TEST_F(PdDirTest, BEH_MAID_ListFolder) {
  ...
  std::string file_hash2_ = "File Hash2";
  std::string ser_mdm2_="", ser_dm2_="";
  PrepareMetaDataMap(-2, file_name2_, EMPTY_FILE, file_hash2_, "Stats2", "Tag2", 0, 3, 4, 0, 0, ser_mdm2_);
  PrepareDataMap(file_hash2_, ser_dm2_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm2_, ser_dm2_)) << "File2 was not added to db1.";
  ...
  std::string file_hash3_ = "File Hash3";
  std::string ser_mdm3_="", ser_dm3_="";
  PrepareMetaDataMap(-2, file_name3_, SMALL_FILE, file_hash3_, "Stats3", "Tag3", 0, 5, 6, 0, 0, ser_mdm3_);
  PrepareDataMap(file_hash3_, ser_dm3_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm3_, ser_dm3_)) << "File3 was not added to db1.";
  ...
  PrepareMetaDataMap(-2, dir_name1_, DIRECTORY, "", "Stats1", "Tag1", 0, 0, 1, 0, 0, ser_mdm4_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm4_, "", dir_key1_)) << "Directory1 was not added to db1.";

  // add directory2
  std::string dir_name2_ = "Directory2";
  std::string dir_key2_="Dir Key2";
  std::string ser_mdm5_="";
  PrepareMetaDataMap(-2, dir_name2_, EMPTY_DIRECTORY, "", "Stats2", "Tag2", 0, 0, 2, 0, 0, ser_mdm5_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm5_, "", dir_key2_)) << "Directory2 was not added to db1.";

  // add directory3
  std::string dir_name3_ = "Directory3";
  std::string dir_key3_="Dir Key3";
  std::string ser_mdm6_="";
  PrepareMetaDataMap(-2, dir_name3_, DIRECTORY, "", "Stats3", "Tag3", 0, 0, 3, 0, 0, ser_mdm6_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm6_, "", dir_key3_)) << "Directory3 was not added to db1.";
  ...
  ASSERT_EQ(static_cast<unsigned int>(6), children_.size()) << "List size incorrect.";
  ASSERT_EQ(EMPTY_DIRECTORY, children_[dir_name2_]) << "Error retrieving dir2 type.";
  ASSERT_EQ(SMALL_FILE, children_[file_name3_]) << "Error retrieving file3 type.";
  ASSERT_EQ(REGULAR_FILE, children_[file_name1_]) << "Error retrieving file1 type.";
  ...
  ASSERT_EQ(EMPTY_FILE, children_[file_name2_]) << "Error retrieving file2 type.";
}
...
TEST_F(PdDirTest, BEH_MAID_ListSubDirs) {
  ...
  ASSERT_EQ(static_cast<unsigned int>(3), subdirs_.size()) << "List size incorrect.";
}
...
TEST_F(PdDirTest, BEH_MAID_GetDmFromHash) {
  ...
  // get serialised Dms
  std::string ser_dm_recovered1_="", ser_dm_recovered2_="", ser_dm_recovered3_="", ser_dm_non_existant_="";
  ...
  ASSERT_TRUE(da_->GetDataMapFromHash("Non-existant File Hash", &ser_dm_non_existant_)<0)\
...
TEST_F(PdDirTest, BEH_MAID_GetDmFromFileName) {
  ...
  ASSERT_EQ(0, da_->GetDataMap(file_name2_, &ser_dm_recovered2_))\
    << "Retrieved ser_dm for file2 incorrectly.";
  ASSERT_EQ(ser_dm2_, ser_dm_recovered2_) << "Returned wrong ser_dm for file2.";
  ASSERT_TRUE(da_->GetDataMap("Non-existant File", &ser_dm_non_existant_)<0)\
    << "Returned ser_dm for file which doesn't exist.";
}
...
TEST_F(PdDirTest, BEH_MAID_GetMdm) {
  ...
  std::string dir_key3_="Dir Key3";
  std::string ser_mdm3_="";
  PrepareMetaDataMap(-2, dir_name3_, DIRECTORY, "", "Stats3", "Tag3", 0, 0, 5, 0, 0, ser_mdm3_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm3_, "", dir_key3_)) << "Directory3 was not added to db1.";

  // get serialised Mdms
  std::string ser_mdm_recovered1_="", ser_mdm_recovered2_="", ser_mdm_recovered3_="", ser_mdm_non_existant_="";
  ...
  ASSERT_TRUE(da_->GetMetaDataMap("Non-existant File", &ser_mdm_non_existant_)<0)\
    << "Returned ser_mdm for file which doesn't exist.";

  // parse Mdms and check
  MetaDataMap mdm1_, mdm2_, mdm3_, mdm_recovered1_, mdm_recovered2_, mdm_recovered3_;
  ...
  ASSERT_TRUE(mdm_recovered1_.ParseFromString(ser_mdm_recovered1_)) << "Couldn't parse mdm_recovered1_.";
  ASSERT_TRUE(mdm_recovered2_.ParseFromString(ser_mdm_recovered2_)) << "Couldn't parse mdm_recovered2_.";
  ASSERT_TRUE(mdm_recovered3_.ParseFromString(ser_mdm_recovered3_)) << "Couldn't parse mdm_recovered3_.";
...
TEST_F(PdDirTest, BEH_MAID_ModifyMdm) {
  ...
  PrepareMetaDataMap(-2, file_name1_, SMALL_FILE, file_hash_new1_,
    "New Stats1", "New Tag1", 1, 2, 3, 0, 0, ser_mdm_new1_);
  PrepareDataMap(file_hash_new1_, ser_dm_new1_);
  ASSERT_EQ(0, da_->ModifyMetaDataMap(ser_mdm_new1_, ser_dm_new1_)) << "File1 Mdm was not updated.";

  // modify file2
  std::string file_hash_new2_ = "New File Hash2";
  std::string ser_mdm_new2_="", ser_dm_new2_="";
  PrepareMetaDataMap(-2, file_name2_, REGULAR_FILE, file_hash_new2_,
    "New Stats2", "New Tag2", 1, 4, 5, 0, 0, ser_mdm_new2_);
  PrepareDataMap(file_hash_new2_, ser_dm_new2_);
  ASSERT_EQ(0, da_->ModifyMetaDataMap(ser_mdm_new2_, ser_dm_new2_)) << "File2 Mdm was not updated.";

  // modify directory3
  std::string ser_mdm_new3_="";
  PrepareMetaDataMap(-2, dir_name3_, EMPTY_DIRECTORY, "", "New Stats3",
    "New Tag3", 0, 0, 5, 0, 0, ser_mdm_new3_);
  ASSERT_NE(0, da_->ModifyMetaDataMap(ser_mdm_new3_, "")) << "Directory3 Mdm was updated.";

  // get serialised Mdms
  std::string ser_mdm_recovered1_="", ser_mdm_recovered2_="", ser_mdm_recovered3_="";
  ASSERT_EQ(0, da_->GetMetaDataMap(file_name1_, &ser_mdm_recovered1_))\
    << "Retrieved ser_mdm for file1 incorrectly.";
  ASSERT_EQ(0, da_->GetMetaDataMap(file_name2_, &ser_mdm_recovered2_))\
    << "Retrieved ser_mdm for file2 incorrectly.";
  ...
  ASSERT_TRUE(mdm2_.ParseFromString(ser_mdm2_)) << "Couldn't parse mdm2_.";
  ASSERT_TRUE(mdm3_.ParseFromString(ser_mdm3_)) << "Couldn't parse mdm3_.";
  ASSERT_TRUE(mdm_new1_.ParseFromString(ser_mdm_new1_)) << "Couldn't parse mdm_new1_.";
  ASSERT_TRUE(mdm_new2_.ParseFromString(ser_mdm_new2_)) << "Couldn't parse mdm_new2_.";
  ASSERT_TRUE(mdm_new3_.ParseFromString(ser_mdm_new3_)) << "Couldn't parse mdm_new3_.";
  ASSERT_TRUE(mdm_recovered1_.ParseFromString(ser_mdm_recovered1_)) << "Couldn't parse mdm_recovered1_.";
  ASSERT_TRUE(mdm_recovered2_.ParseFromString(ser_mdm_recovered2_)) << "Couldn't parse mdm_recovered2_.";
  ASSERT_TRUE(mdm_recovered3_.ParseFromString(ser_mdm_recovered3_)) << "Couldn't parse mdm_recovered3_.";
  ...
  for (unsigned int i=0; i<mdm_.size(); i++) {
    if (mdm_[i].type() <= 2) { // ie a file, not a dir
      ...
      ASSERT_EQ(mdm_new_[i].file_size_high(), mdm_recovered_[i].file_size_high())\
...
TEST_F(PdDirTest, BEH_MAID_ChangeCtime) {
  ...
  std::string ser_mdm_before1_="", ser_mdm_before2_="", ser_mdm_after1_="", ser_mdm_after2_="";
  MetaDataMap mdm_before1_, mdm_before2_, mdm_after1_, mdm_after2_;
  boost::uint32_t file_time_before_, dir_time_before_, file_time_after_, dir_time_after_;
  ...
  ASSERT_TRUE(mdm_before1_.ParseFromString(ser_mdm_before1_)) << "Couldn't parse mdm_before1_.";
  ASSERT_TRUE(mdm_before2_.ParseFromString(ser_mdm_before2_)) << "Couldn't parse mdm_before2_.";
  ...
  ASSERT_EQ(0, da_->ChangeCtime(file_name_)) << "Failed to change creation time for file.";
  ASSERT_EQ(0, da_->ChangeCtime(dir_name_)) << "Failed to change creation time for dir.";
  ASSERT_NE(0, da_->ChangeCtime("Non-existant File")) << "Changed creation time for non-existant file.";
  ...
  ASSERT_EQ(0, da_->GetMetaDataMap(dir_name_, &ser_mdm_after2_))\
    << "Retrieved ser_mdm for dir incorrectly.";
  ASSERT_TRUE(mdm_after1_.ParseFromString(ser_mdm_after1_)) << "Couldn't parse mdm_after1_.";
  ASSERT_TRUE(mdm_after2_.ParseFromString(ser_mdm_after2_)) << "Couldn't parse mdm_after2_.";
  file_time_after_ = mdm_after1_.creation_time();
  dir_time_after_ = mdm_after2_.creation_time();
  ASSERT_NE(file_time_before_, file_time_after_) << "Creation time for file has not updated.";
  ASSERT_NE(dir_time_before_, dir_time_after_) << "Creation time for file has not updated.";
}
...
TEST_F(PdDirTest, BEH_MAID_ChangeMtime) {
  ...
  ASSERT_EQ(0, da_->ChangeMtime(file_name_)) << "Failed to change last_modified time for file.";
  ASSERT_EQ(0, da_->ChangeMtime(dir_name_)) << "Failed to change last_modified time for dir.";
  ASSERT_NE(0, da_->ChangeMtime("Non-existant File")) << "Changed last_modified time for non-existant file.";
  ...
  ASSERT_NE(file_time_before_, file_time_after_) << "last_modified time for file has not updated.";
  ASSERT_NE(dir_time_before_, dir_time_after_) << "last_modified time for file has not updated.";
}
...
TEST_F(PdDirTest, BEH_MAID_ChangeAtime) {
  ...
  ASSERT_EQ(0, da_->ChangeAtime(file_name_)) << "Failed to change last_access time for file.";
  ASSERT_EQ(0, da_->ChangeAtime(dir_name_)) << "Failed to change last_access time for dir.";
  ASSERT_NE(0, da_->ChangeAtime("Non-existant File")) << "Changed last_access time for non-existant file.";
  ...
  ASSERT_NE(file_time_before_, file_time_after_) << "last_access time for file has not updated.";
  ASSERT_NE(dir_time_before_, dir_time_after_) << "last_access time for file has not updated.";
}
...
TEST_F(PdDirTest, BEH_MAID_ModifyViaAddElement) {
  ...
  std::string dir_key_="Dir Key";
  std::string ser_mdm2_="";
  PrepareMetaDataMap(-2, dir_name_, EMPTY_DIRECTORY, "", "Stats2", "Tag2", 0, 0, 3, 0, 0, ser_mdm2_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm2_, "", dir_key_)) << "Directory was not added to db1.";

  // modify file
  std::string file_hash_new_ = "New File Hash";
  std::string ser_mdm_new1_="", ser_dm_new1_="";
  PrepareMetaDataMap(-2, file_name_, SMALL_FILE, file_hash_new_,
    "New Stats1", "New Tag1", 1, 4, 5, 0, 0, ser_mdm_new1_);
  PrepareDataMap(file_hash_new_, ser_dm_new1_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm_new1_, ser_dm_new1_)) << "File Mdm was not updated.";

  // modify directory
  std::string ser_mdm_new2_="";
  PrepareMetaDataMap(-2, dir_name_, DIRECTORY, "", "New Stats2",
    "New Tag2", 0, 0, 6, 0, 0, ser_mdm_new2_);
  ASSERT_EQ(0, da_->AddElement(ser_mdm_new2_, "")) << "Directory Mdm was not updated.";
  ...
  // check file updated
  ASSERT_EQ(mdm1_.display_name(), mdm_recovered1_.display_name()) << "File display name has changed.";
  ASSERT_EQ(mdm_new1_.display_name(), mdm_recovered1_.display_name())\
    << "File display name has not updated correctly.";

  ASSERT_NE(mdm1_.type(), mdm_recovered1_.type()) << "File type has not updated.";
  ASSERT_EQ(mdm_new1_.type(), mdm_recovered1_.type()) << "File type has not updated correctly.";

  ASSERT_NE(mdm1_.file_hash(0), mdm_recovered1_.file_hash(0)) << "File hash has not updated.";
  ASSERT_EQ(mdm_new1_.file_hash(0), mdm_recovered1_.file_hash(0)) << "File hash has not updated correctly.";

  ASSERT_NE(mdm1_.stats(), mdm_recovered1_.stats()) << "File stats have not updated.";
  ASSERT_EQ(mdm_new1_.stats(), mdm_recovered1_.stats()) << "File stats have not updated correctly.";

  ASSERT_NE(mdm1_.tag(), mdm_recovered1_.tag()) << "File tag has not updated.";
  ASSERT_EQ(mdm_new1_.tag(), mdm_recovered1_.tag()) << "File tag has not updated correctly.";

  ASSERT_NE(mdm1_.file_size_high(), mdm_recovered1_.file_size_high()) << "File file_size_high has not updated.";
  ASSERT_EQ(mdm_new1_.file_size_high(), mdm_recovered1_.file_size_high())
    << "File file_size_high has not updated correctly.";

  ASSERT_NE(mdm1_.file_size_low(), mdm_recovered1_.file_size_low()) << "File file_size_low has not updated.";
  ASSERT_EQ(mdm_new1_.file_size_low(), mdm_recovered1_.file_size_low())\
    << "File file_size_low has not updated correctly.";

  ASSERT_EQ(mdm1_.creation_time(), mdm_recovered1_.creation_time()) << "File creation time has changed.";
  ASSERT_NE(mdm_new1_.creation_time(), mdm_recovered1_.creation_time())\
    << "File creation time has not updated correctly.";

  // check dir updated
  ASSERT_EQ(mdm2_.display_name(), mdm_recovered2_.display_name()) << "Dir display name has changed.";
  ASSERT_EQ(mdm_new2_.display_name(), mdm_recovered2_.display_name())\
    << "Dir display name has not updated correctly.";

  ASSERT_NE(mdm2_.type(), mdm_recovered2_.type()) << "Dir type has not updated.";
  ASSERT_EQ(mdm_new2_.type(), mdm_recovered2_.type()) << "Dir type has not updated correctly.";

  ASSERT_NE(mdm2_.stats(), mdm_recovered2_.stats()) << "Dir stats have not updated.";
  ASSERT_EQ(mdm_new2_.stats(), mdm_recovered2_.stats()) << "Dir stats have not updated correctly.";

  ASSERT_NE(mdm2_.tag(), mdm_recovered2_.tag()) << "Dir tag has not updated.";
  ASSERT_EQ(mdm_new2_.tag(), mdm_recovered2_.tag()) << "Dir tag has not updated correctly.";

  ASSERT_EQ(mdm2_.creation_time(), mdm_recovered2_.creation_time()) << "Dir creation time has changed.";
