...
class FSLinux {
  ...
  public:
    FSLinux();
    FSLinux &operator=(const fs_l_fuse::FSLinux &rhs);
    explicit FSLinux(const fs_l_fuse::FSLinux &rhs);
    ~FSLinux();
    ...
    static int ms_setxattr(const char *path, const char *, const char *,
      size_t, int);
    ...
    static int ms_readdir(const char *path, void *buf, fuse_fill_dir_t
